...
class MailClient:
    ...
    def show_header(self, number, mail=None):
        if mail is None:
            try:
        ...
        with MailClient(address, password, host, port,
            ...
            while True:
                ...
                if command == 'quit':
                    ...
                    if length != 2:
                        continue
                    n = min(int(user_input[1]), client.msg_count)
                    if not client.check_mail_number(n):
                        continue
                    for i in range(1, n + 1):
                        client.show_header(i)
                        sys.stdout.write('=' * 50 + '\n')
                elif command == 'help':
                    if length != 1:
                        continue
